...
void td_esc_lalt_lctl_spotlight_emoji_finished(qk_tap_dance_state_t *state, void *user_data) {
...
void td_esc_lalt_lctl_spotlight_emoji_reset(qk_tap_dance_state_t *state, void *user_data) {
    switch (xtap_state.state) {
...
void td_esc_lctl_lalt_windows_emoji_finished(qk_tap_dance_state_t *state, void *user_data) {
    xtap_state.state = cur_dance(state);
    switch (xtap_state.state) {
...
void td_esc_lctl_lalt_windows_emoji_reset(qk_tap_dance_state_t *state, void *user_data) {
    switch (xtap_state.state) {
        case TD_SINGLE_TAP: unregister_code(KC_ESC); break;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [4] = LAYOUT_ortho_3x10_4(
        ...
        KC_A,         KC_R, KC_S, KC_T, KC_D,                         KC_H,   KC_N,          KC_E,         KC_I,   KC_O,
        LSFT_T(KC_Z), KC_X, KC_C, KC_V, KC_B,                         KC_K,   KC_M,          KC_COMMA,     KC_DOT, RSFT_T(KC_SLSH),
                                  TD(TD_ESC_LCTL_LALT_WINDOWS_EMOJI), KC_SPC, LT(5, KC_TAB), LT(6, KC_ENT)
    ),
